...
void displayPCRecords(pcRecord PCList[]); //PARTIAL
...
int main(void) {
    ...
    /*Problem 4*/
    printf("\nPROBLEM 4:\n");
    //WIP: Initialize a new pcList occLB445 using genPCListOccupied().
    ...
    //DONE:Call displayPCList() passing LB445 as the parameter.
    displayPCList(LB445);
    //WIP: Initialize a new array of pcRecords using removeDamagedPCs().
...
void displayPCList(pcList desktops)
{
    int x;
    for(x = 0; x < desktops.count; x++){
        printf("PC[%d]\nState: %s\nCurrent User: %s, \nLast User: %s\n\n", desktops.PC[x].pcNumber, displayState(desktops.PC[x].pcState), desktops.PC[x].currentUser, desktops.PC[x].lastUser);
    }
...
void vacatePC(pcList* desktops, int pcNumber)
{
    desktops->PC[pcNumber] = (pcRecord) {
        {"XXXXXXXX", "XXXXXXXX", "XXXXXXXX"}, .pcState = VACANT};
    desktops->occupied--;
}
...
{
    int i;
    for(i = 0; PCList[i].pcNumber < 30; i++){
        printf("PC[%d]\nState: %s\nCurrent User: %s, \nLast User: %s\n\n", PCList[i].pcNumber, displayState(PCList[i].pcState), PCList[i].currentUser, PCList[i].lastUser);
    }
}